#!/usr/bin/env bash
# generate_no_lyrics_v4.5.sh — render no_lyrics_v4.5.md via suno-pp-cli
# Keep in sync with no_lyrics_v4.5.md — Title/Lyrics/Style/Exclude are
# extracted live from the markdown; settings below mirror its table.
# Style variant: the ★ Recommended (A) by default; override with
#   VARIANT=A|C ./generate_no_lyrics_v4.5.sh
set -euo pipefail

PACK_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROMPT_FILE="$PACK_DIR/no_lyrics_v4.5.md"
PROMPT_NAME="no_lyrics_v4.5"
...
VARIANT="${VARIANT:-}"
VARIANT_LABEL="${VARIANT:-A (★ Recommended)}"
...
==================================================================
 ...
 Note        : --exclude is passed for forward-compat, but the CLI
               ...
               defense rests on the toggle + clean style + bracket
               structure tags (all of which transmit). v4.5 is the
               safer instrumental model — fewer hallucinated vocals.
==================================================================
BANNER
