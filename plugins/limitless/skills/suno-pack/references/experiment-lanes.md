...
Pack authoring stays free: `experiment_map.md` (all five lanes on paper)
ships with every experimental pack and doubles as the `list` menu
source. Track the per-pack running credit total across run logs.
...
## experiment_map.md template

```markdown
# <Track Name> — Experiment Map

*<date> — suno-pack experimental mode · seed: <clip id or "faithful pack pending">*

| # | Lane | Mutation axis | One-liner | Cost | Requires |
|---|---|---|---|---|---|
| 1 | Genre Transposition | genre, wholesale | the song in an alien body | ~10 cr | seed clip |
| 2 | Controlled Friction | emotional architecture | contradiction as depth | ~10 cr | — |
| 3 | Alternate-Reality Renders | performance reality | the unplugged B-side | ~10 cr | seed keeper |
| 4 | Red-Letter Mining | weirdness ceiling | prospect the chaos | ~10 cr | — |
| 5 | Emotion Lexicon | style vocabulary | one rare word does the work | ~10 cr | — |

Scorecard (human fills after listening): keep = concept survival ≥3 AND
surprise ≥4 AND artifact cost ≤3.

| Roll | Lane | Clip | Concept | Surprise | Cost | Keep? | Note |
|---|---|---|---|---|---|---|---|
